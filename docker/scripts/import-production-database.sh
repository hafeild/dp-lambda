...
docker-compose -f docker/Compose.prod.yml up -V --build --force-recreate -d alice_db

dir=$(mktemp -p . -d data-dump-XXXX);
mv $databaseDumpFile $dir/data.sql

docker-compose -f docker/Compose.prod.yml run --rm -v "$PWD/$dir":/data -w /data alice_db \
